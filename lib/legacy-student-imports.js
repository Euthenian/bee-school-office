...
export function readLegacyStudentWorkbook(filePath, options = {}) {
  const buffer = readFileSync(filePath);
  const extension = extname(filePath).toLowerCase();
  if (extension === ".csv" && (options.sheetName || options.sheetNames?.length)) {
    throw new Error("Worksheet selection requires an XLSX or XLSM workbook.");
  }
  const workbook = extension === ".csv" ? parseCsvWorkbook(buffer, filePath) : parseXlsxWorkbook(buffer, filePath, options);
...
export function parseXlsxWorkbook(buffer, filePath = "workbook.xlsx", options = {}) {
  ...
  const requestedNames = new Set(options.sheetName ? [options.sheetName] : options.sheetNames || []);
  const workbookSheets = parseWorkbookSheets(workbookXml);
  for (const name of requestedNames) {
    if (!workbookSheets.some((sheet) => sheet.name === name)) {
      throw new Error(`Requested worksheet was not found: ${name}.`);
    }
  }
  // Select from workbook metadata before reading any worksheet payload. The ZIP
  // index below is lazy, so unrelated worksheets are never decompressed.
  const selectedSheets = workbookSheets.filter((sheet) => !requestedNames.size || requestedNames.has(sheet.name));
  const sheetRelationships = parseWorkbookRelationships(relationshipXml);
  const sources = selectedSheets
    .map((sheet) => {
      const target = sheetRelationships.get(sheet.relationshipId);
      if (!target) {
        if (requestedNames.size) throw new Error(`Requested worksheet has no workbook relationship: ${sheet.name}.`);
        return null;
      }
      return { ...sheet, xml: readZipText(entries, normalizeWorkbookTarget(target)) };
    })
    .filter(Boolean);
  const sharedIndexes = new Set();
  for (const source of sources) {
    for (const cell of source.xml.matchAll(/<c\b([^>]*?)(?:\/>|>([\s\S]*?)<\/c>)/g)) {
      if (parseAttributes(cell[1]).t !== "s") continue;
      const value = cell[2]?.match(/<v[^>]*>([\s\S]*?)<\/v>/)?.[1];
      if (value !== undefined) sharedIndexes.add(Number(value));
    }
  }
  // Shared strings are workbook-wide infrastructure. Decode only the indexes
  // actually referenced by selected worksheets, never unrelated cell values.
  const sharedStrings = sharedIndexes.size && entries.has("xl/sharedStrings.xml")
    ? parseSharedStrings(readZipText(entries, "xl/sharedStrings.xml"), sharedIndexes)
    : [];
  const sheets = sources.map((source) => parseWorksheet(source.xml, source.name, sharedStrings));

  return {
    file: { name: basename(filePath), path: filePath },
    date1904: ["1", "true"].includes(parseAttributes(workbookXml.match(/<workbookPr\b([^>]*)/)?.[1] || "").date1904),
    sheets
  };
}
...
function parseWorksheet(xml, sheetName, sharedStrings) {
  ...
  const metadata = {
    dimension: parseAttributes(xml.match(/<dimension\b([^>]*)/)?.[1] || "").ref || null,
    physicalRowCount: rows.length,
    physicalDataRowCount: headerRow ? rows.filter((row) => row.rowNumber > headerRow.rowNumber).length : 0,
    firstPhysicalRowNumber: rows[0]?.rowNumber ?? null,
    lastPhysicalRowNumber: rows.at(-1)?.rowNumber ?? null,
    headerRowNumber: headerRow?.rowNumber ?? null
  };
  if (!headerRow) return { name: sheetName, headers: [], columns: [], rows: [], ...metadata };

  // Retain unlabelled populated columns and disambiguate repeated labels so raw
  // source data cannot disappear through Object.fromEntries key collisions.
  const width = rows.reduce((maximum, row) => Math.max(maximum, row.cells.length), 0);
  const usedHeaders = new Set();
  const columns = Array.from({ length: width }, (_, index) => {
    const sourceHeader = headerRow.cells[index]?.value ?? "";
    const label = normalizeHeader(sourceHeader) || `Column${index + 1}`;
    let header = label;
    let occurrence = 1;
    while (usedHeaders.has(header)) header = `${label} [${++occurrence}]`;
    usedHeaders.add(header);
    return { column_index: index + 1, column_letter: columnNameFromIndex(index), source_header: sourceHeader, header };
  });
  ...
  return { name: sheetName, headers, columns, rows: dataRows, ...metadata };
}
...
function parseSharedStrings(xml, requiredIndexes) {
  const strings = [];
  ...
  while ((match = sharedStringRegex.exec(xml))) {
    ...
    if (requiredIndexes && !requiredIndexes.has(stringIndex)) continue;
...
function readZipEntries(buffer) {
  ...
  while (offset < endOffset) {
    ...
    // Index only the archive metadata here. Extracting eagerly would process
    // every worksheet even when the caller explicitly selected only Taiken.
    entries.set(fileName, { buffer, compressionMethod, compressedSize, localHeaderOffset });
...
function readZipText(entries, name) {
  ...
  const { buffer, localHeaderOffset, compressedSize, compressionMethod } = entry;
  ...
  return (compressionMethod === 0 ? compressed : inflateRawSync(compressed)).toString("utf8");
}
