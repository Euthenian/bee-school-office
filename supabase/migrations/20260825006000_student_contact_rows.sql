with ranked_primary_contacts as (
  select
    id,
    row_number() over (
      partition by student_id, contact_type
...
set is_primary = false,
    updated_at = now()
from ranked_primary_contacts rpc
where sc.id = rpc.id
  and rpc.primary_rank > 1;

create unique index if not exists student_contacts_one_primary_per_type_idx
