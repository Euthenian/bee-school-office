...
export function getMissingTrialLessonConversionFields(trialLesson) {
  const missing = [];
  if (!trialLesson?.lesson_type) missing.push("lessonType");
  if (!trialLesson?.level_id && !trialLesson?.class_levels?.id) missing.push("levelId");
  return missing;
}
...
export function getNearestUpcomingTrialLesson(trialLessons = [], today = getLocalDateKey()) {
  return [...trialLessons]
    .filter((trialLesson) => trialLesson?.trial_date && trialLesson.trial_date >= today)
    .sort(compareTrialLessonSchedule)[0] || null;
}

export function formatUpcomingTrialLessonDate(trialLesson) {
  const date = parseDateOnly(trialLesson?.trial_date);
  if (!date) return "";
...
function compareTrialLessonSchedule(left, right) {
  const dateComparison = compareNullable(left?.trial_date, right?.trial_date, 1, parseDateOnly);
  if (dateComparison !== 0) return dateComparison;

  return compareNullable(left?.trial_time, right?.trial_time, 1);
}
