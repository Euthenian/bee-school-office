...
export function createStudentFinanceForm(finance = {}, bankDetails = {}) {
  ...
  return {
    billingPlanId: normalizedFinance.billing_plan_id || "",
    monthlyFeeYen:
    ...
    billingStartDate: normalizedFinance.billing_start_date || "",
    billingEndDate: normalizedFinance.billing_end_date || "",
...
export function validateStudentFinanceForm(form = {}, options = {}) {
  ...
  if (form.billingStartDate && form.billingEndDate && form.billingEndDate < form.billingStartDate) {
    return "Billing end date cannot be before billing start date.";
  }
...
export function hasStudentFinanceData(finance = {}) {
  ...
  return Boolean(
    finance.billing_plan_id ||
      (finance.monthly_fee_yen !== null && finance.monthly_fee_yen !== undefined) ||
      finance.billing_start_date ||
      finance.billing_end_date ||
...
export function normalizeStudentFinancePayload(form = {}, options = {}) {
  const payload = {
    billingPlanId: emptyToNull(form.billingPlanId),
    monthlyFeeYen: form.monthlyFeeYen === "" ? null : Number(form.monthlyFeeYen),
    billingStartDate: emptyToNull(form.billingStartDate),
    billingEndDate: emptyToNull(form.billingEndDate),
