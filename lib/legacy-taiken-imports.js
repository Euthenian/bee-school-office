...
export function matchTaikenStudent({ names = [], email, phone, legacyCustomerId }, students, target) {
  ...
  for (const student of students) {
    ...
    if (signals.length) matches.push({ student_id: student.id, signals, strong: signals.includes("legacy_customer_id") || (signals.includes("exact_name") && (signals.includes("email") || signals.includes("phone"))) });
  }
...
export function interpretTaikenStatus(raw, date1904 = false) {
  ...
  if (conflict || unknown.length) return { status: null, joined_candidate: joinedCandidate, confirmed_joined: false, reason: "conflicting_or_unknown_conversion_evidence", refusal_date: refusalDate, possible_join_dates: dates };
  ...
  if (dates.length) return { status: "joined", joined_candidate: true, confirmed_joined: false, reason: "date_in_joined_field_requires_owner_confirmation", refusal_date: refusalDate, possible_join_dates: dates };
...
function normalizeRow(source, workbook, snapshot, teacherMappings) {
  ...
  if (!prospect.japanese_name) blockers.push("missing_prospect_name");
  const participants = [];
  ...
  for (const slot of [1, 2]) {
    ...
    if (multipleNames) {
      ...
      if (!blockers.includes("participant_household_needs_review")) blockers.push("participant_household_needs_review");
  ...
  if (!status.status || (status.joined_candidate && !status.confirmed_joined)) blockers.push(status.reason);
  ...
  for (const field of ["trial_date", "trial_time", "lesson_type", "level_id"]) if (!trial[field]) blockers.push(`missing_or_unresolved_${field}`);
  ...
  if (householdEvidence || blockers.includes("participant_household_needs_review")) {
    warn("multiple_participants_require_identity_review", "name of student 1 / Age group S1 / notes");
    blockers.push("participant_household_needs_review");
    if (category === "A") {
      category = "B";
      matching.category = "B";
  ...
  if (status.confirmed_joined && !blockers.includes("participant_household_needs_review")) {
  ...
  const row = {
    ...
    import_status: "dry_run", validation_state: blockers.length ? "error" : warnings.length ? "warning" : "valid"
  };
  return row;
}
...
export function buildLegacyTaikenDryRun({ workbook, snapshot, teacherMappings = {} }) {
  ...
  const joined = rows.filter((row) => row.normalized_candidate.status_evidence.joined_candidate);
  ...
  const blockerCounts = {};
  for (const row of rows) for (const blocker of row.normalized_candidate.import_blockers) blockerCounts[blocker] = (blockerCounts[blocker] || 0) + 1;
  return {
    ...
    summary: {
      ...
      rows_with_multiple_participant_evidence: count((row) => row.normalized_candidate.import_blockers.includes("participant_household_needs_review")),
      ...
      joined_candidates: joined.length, confirmed_joined_rows: count((row) => row.normalized_candidate.status_evidence.confirmed_joined),
      ...
      duplicate_candidate_groups: duplicates, rows_with_import_blockers: count((row) => row.normalized_candidate.import_blockers.length > 0),
...
export function productionFieldMapping() {
  return [
    { source: "Name", target: "prospects.japanese_name; alphabet_name only for Latin-script source names", rule: "Preserve contact name as written. Missing required name blocks production." },
    ...
    { source: "day of taiken / Time of Taiken", target: "trial_lessons.trial_date / trial_time", rule: "Strict validated calendar date and time; missing/invalid blocks current production schema." },
    { source: "Type of lesson / コース", target: "trial_lessons.lesson_type", rule: "Explicit group/private only. Clear course type used only when Type of lesson is blank; conflicting values block." },
    { source: "Age group S1", target: "trial_lessons.level_id; named participant 1 age_group_level_id", rule: "Exact canonical age-group mapping only, no inference from numeric age." },
    ...
    { source: "name of student 1 / name of student 2", target: "trial_lesson_participants rows: japanese_name, alphabet_name, furigana, age_group_level_id, requested_level_id, converted_student_id", rule: "One row per explicit named participant; missing names and mixed households require review." },
    ...
    { source: "Joined / Joined2 / refusal date", target: "trial_lessons.status", rule: "See per-row status evidence and correlations. Never infer completed from a historical scheduled date." },
    { source: "existing Student match", target: "trial_lessons.converted_student_id; named participant converted_student_id", rule: "A only plus confirmed conversion; no Student creation, merge or update." },
