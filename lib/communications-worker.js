...
export async function processQueuedCommunicationActions({
  ...
  for (const action of actions) {
    ...
    try {
      const result = await processIntegrationAction({ action, calendarProvider, emailProvider, repository, resendProvider });
...
export function createResendSenderClient({ apiKey, from }, fetchImpl = fetch) {
  assertNonEmpty(apiKey, "RESEND_API_KEY is required for Resend email delivery.");
  assertNonEmpty(from, "AI_EIGO_INVITATION_EMAIL_FROM is required for Resend email delivery.");
...
async function processIntegrationAction({ action, calendarProvider, emailProvider, repository, resendProvider }) {
  ...
  if (action.provider === "resend" && action.action_type === "send_email") {
    ...
    return resendProvider.sendEmail(emailPayload);
  }
...
function getMissingSecretsForAction(action, config) {
  ...
  if (action.provider === "resend" && action.action_type === "send_email") {
    return config.resendReady ? [] : config.missingResendSecrets || [];
  }
