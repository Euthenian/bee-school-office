import { normalizeAiEigoInvitationResponse } from "./ai-eigo-invitations.js";
import { getDefaultStudentEmail } from "./communication-templates.js";
...
import { buildBillingPlanMutation, normalizeBillingPlan } from "./billing-plans.js";
import { buildFinancialDocumentRuleInsert } from "./financial-documents.js";
import { buildMonthlyBillingSnapshotUpdate, normalizeMonthlyBillingRow } from "./monthly-billing.js";
import { buildOfficeTodoInsert } from "./office-todos.js";
import { normalizeExpenseAmount } from "./expenses.js";
import { normalizePayrollAmount } from "./payroll.js";
import { canCreateStudents } from "./roles.js";
import { serializeStaffAssignments } from "./staff.js";
import { normalizeStudentFinance, normalizeStudentFinancePayload, normalizeStudentBankDetails } from "./student-finance.js";
import { serializeGuardianRows, serializeNoteRows } from "./student-form.js";
import { isValidStudentStatus } from "./students.js";
import {
...
export const studentAiEigoInvitationSelect = `
  id,
  communication_id,
  recipient_email,
  status,
...
export const studentAiEigoLinkSelect = `
  id,
  ai_eigo_user_id,
  ai_eigo_email,
  entitlement_code,
...
export const studentListSelect = `
  ...
  ai_eigo_student_links:ai_eigo_student_links!ai_eigo_student_links_student_id_organization_id_school_id_fkey (
...
export const classSelect = `
  ...
  created_at,
  updated_at,
  schools:schools!classes_school_id_organization_id_fkey (
    id,
    name,
    status,
    organizations (
  ...
  student_enrollments:student_enrollments!student_enrollments_class_id_organization_id_school_id_fkey (
    id,
    student_id,
    status,
    level,
...
export const classProfileSelect = `
  id,
  organization_id,
  school_id,
  assigned_teacher_profile_id,
  ...
  student_enrollments:student_enrollments!student_enrollments_class_id_organization_id_school_id_fkey (
    ...
    start_date,
    end_date,
    students:students!student_enrollments_student_id_organization_id_school_id_fkey (
      id,
      first_name,
      last_name,
      preferred_name,
...
export const recurringExpenseTemplateSelect = `
  id,
  organization_id,
  school_id,
  category_id,
  ...
  created_at,
  updated_at,
  schools:schools!recurring_expense_templates_school_id_organization_id_fkey (
    id,
    name,
    status
  ),
  expense_categories:expense_categories!recurring_expense_templates_category_id_organization_id_fkey (
    id,
    name,
    code,
    status
...
export const financialDocumentSelect = `
  id,
  organization_id,
  school_id,
  source,
  ...
  created_at,
  updated_at,
  schools:schools!financial_documents_school_id_organization_id_fkey (
    id,
    name,
    status
  ),
  expense_categories:expense_categories!financial_documents_category_id_organization_id_fkey (
    id,
    name,
    code,
    status
  ),
  linked_expense:expenses!financial_documents_linked_expense_id_org_school_fkey (
    id,
    description,
    amount,
    currency,
...
export const financialDocumentRuleSelect = `
  id,
  organization_id,
  school_id,
  sender_domain,
  ...
  created_at,
  updated_at,
  schools:schools!financial_document_rules_school_id_organization_id_fkey (
    id,
    name,
    status
  ),
  expense_categories:expense_categories!financial_document_rules_category_id_organization_id_fkey (
    id,
    name,
    code,
    status
...
export const officeTodoSelect = `
  ...
  created_at,
  updated_at,
  schools:schools!office_todo_items_school_id_organization_id_fkey (
    id,
    name,
    status
  ),
  assigned_profile:profiles!office_todo_items_assigned_profile_id_fkey (
    id,
    full_name,
    email
  )
...
export async function fetchDashboardData(supabase, options = {}) {
  try {
    const [activeStudents, inactiveStudents, totalStudents, schools, staff, recentStudents, cronHealthResult] = await Promise.all([
      countRows(supabase, "students", { column: "status", value: "active" }),
      countRows(supabase, "students", { column: "status", value: "inactive" }),
...
export async function fetchStudentMonthlyBillingSnapshots(supabase, filters = {}) {
  const { data, error } = await supabase.rpc("get_student_monthly_billing_snapshots_mvp", {
    p_billing_month: emptyToNull(filters.billingMonth),
    p_organization_id: emptyToNull(filters.organizationId),
    p_school_id: emptyToNull(filters.schoolId)
  });

  return {
    data: (data || []).map(normalizeMonthlyBillingRow),
    error
  };
}

export async function fetchStudentMonthlyBillingStopAlerts(supabase, filters = {}) {
  const { data, error } = await supabase.rpc("get_student_monthly_billing_stop_alerts_mvp", {
    p_organization_id: emptyToNull(filters.organizationId),
    p_reference_month: emptyToNull(filters.billingMonth),
    p_school_id: emptyToNull(filters.schoolId)
  });
...
export async function createStudentMonthlyBillingSnapshots(supabase, input = {}) {
  const { data, error } = await supabase.rpc("create_student_monthly_billing_snapshots_mvp", {
    p_billing_month: emptyToNull(input.billingMonth),
    p_organization_id: emptyToNull(input.organizationId),
...
export async function updateStudentMonthlyBillingSnapshot(supabase, snapshotId, input = {}) {
  const payload = buildMonthlyBillingSnapshotUpdate(input);
  const { data, error } = await supabase.rpc("update_student_monthly_billing_snapshot_mvp", {
    p_comment: payload.comment,
    p_final_amount: payload.finalAmount,
...
export async function fetchStudents(supabase, input = "") {
  ...
  if (trimmed) {
    const term = `%${trimmed}%`;
    query = query.or(`first_name.ilike.${term},last_name.ilike.${term},preferred_name.ilike.${term},legacy_customer_id.ilike.${term},legacy_japanese_name.ilike.${term}`);
  }
...
export async function sendAiEigoStudentInvitation(supabase, studentId) {
  const { data, error } = await supabase.rpc("send_ai_eigo_student_invitation_mvp", {
    p_student_id: studentId
  });

  return { data: normalizeAiEigoInvitationResponse(data), error };
}
...
export async function fetchClasses(supabase, filters = {}) {
  let query = supabase
    .from("classes")
    .select(classSelect)
...
export async function fetchClassOptions(supabase, schoolId, options = {}) {
  if (!schoolId) return { data: [], error: null };

  const { data, error } = await fetchClasses(supabase, {
    schoolId,
    status: options.includeInactive ? "all" : "active"
  });
  if (error) return { data: [], error };

  const currentClassId = options.currentClassId || "";
  const classes = (data || []).filter((classRow) => classRow.status === "active" || classRow.id === currentClassId);
  return { data: classes, error: null };
}

export async function fetchClassProfile(supabase, classId) {
  const { data, error } = await supabase
    .from("classes")
    .select(classProfileSelect)
    .eq("id", classId)
    .maybeSingle();
...
export async function createClass(supabase, input) {
  const { data, error } = await supabase.rpc("create_class_mvp", {
...
export async function updateClass(supabase, input) {
  const { data, error } = await supabase.rpc("update_class_mvp", {
    p_assigned_teacher_profile_id: emptyToNull(input.assignedTeacherProfileId),
...
export async function deleteClass(supabase, classId) {
  const { data, error } = await supabase.rpc("delete_class_mvp", {
    p_class_id: classId
  });
...
export async function fetchEligibleTeachersBySchools(supabase, schoolIds = []) {
  const uniqueSchoolIds = [...new Set((schoolIds || []).filter(Boolean))];
  if (!uniqueSchoolIds.length) return { data: {}, error: null };

  const results = await Promise.all(
    uniqueSchoolIds.map(async (schoolId) => {
      const result = await fetchSchoolTeachers(supabase, schoolId);
      return { schoolId, ...result };
    })
  );

  return {
    data: results.reduce((accumulator, result) => {
      accumulator[result.schoolId] = result.data || [];
      return accumulator;
    }, {}),
...
export async function fetchTeacherClassCounts(supabase, profileIds = []) {
  const uniqueProfileIds = [...new Set((profileIds || []).filter(Boolean))];
  if (!uniqueProfileIds.length) return { data: {}, error: null };

  const { data, error } = await supabase
    .from("classes")
    .select("id, assigned_teacher_profile_id")
    .eq("status", "active")
    .in("assigned_teacher_profile_id", uniqueProfileIds);

  if (error) return { data: {}, error };

  return {
    data: (data || []).reduce((accumulator, row) => {
      const profileId = row.assigned_teacher_profile_id;
      accumulator[profileId] = (accumulator[profileId] || 0) + 1;
      return accumulator;
    }, {}),
    error: null
  };
...
export async function fetchTeacherAssignedClasses(supabase, profileId) {
  ...
  const { data, error } = await supabase
    ...
    .eq("assigned_teacher_profile_id", profileId)
    .order("lesson_day", { ascending: true })
    .order("lesson_time", { ascending: true });
...
export async function fetchTeacherUpcomingTrialLessons(supabase, profileId) {
  if (!profileId) return { data: [], error: null };
  ...
  const { data, error } = await supabase
    ...
    .eq("assigned_teacher_profile_id", profileId)
    .gte("trial_date", getTodayDateString())
...
export async function fetchStudentBillingPlanOptions(supabase, studentId) {
  const { data, error } = await supabase.rpc("get_student_billing_plan_options_mvp", {
    p_student_id: studentId
  });

  return {
    data: (data || []).map(normalizeBillingPlan),
    error
  };
}

export async function fetchBillingPlans(supabase, filters = {}) {
  let query = supabase
    .from("billing_plans")
    .select(
      "id, organization_id, school_id, name, lesson_type, lesson_duration_minutes, lessons_per_month, monthly_fee_yen, active, sort_order, created_at, updated_at"
    )
    .order("sort_order", { ascending: true })
    .order("name", { ascending: true });
  ...
  const { data, error } = await query;
  return { data: (data || []).map(normalizeBillingPlan), error };
}

export async function createBillingPlan(supabase, form) {
  const row = buildBillingPlanMutation(form);
  const { data, error } = await supabase
    .from("billing_plans")
    .insert(row)
    .select(
  ...
  return { data: data ? normalizeBillingPlan(data) : null, error, row };
}

export async function updateBillingPlan(supabase, planId, form) {
  const row = buildBillingPlanMutation(form);
  const { data, error } = await supabase
    .from("billing_plans")
    .update(row)
...
export async function setBillingPlanActive(supabase, planId, active) {
  const { data, error } = await supabase
    .from("billing_plans")
  ...
  return { data: data ? normalizeBillingPlan(data) : null, error };
}

export async function fetchStudentFinance(supabase, studentId) {
  const { data, error } = await supabase.rpc("get_student_finance_mvp", {
    p_student_id: studentId
  });
  const row = Array.isArray(data) ? data[0] : data;

  return {
    data: row ? normalizeStudentFinance(row) : null,
    error
  };
}

export async function fetchStudentBankDetails(supabase, studentId) {
  const { data, error } = await supabase.rpc("get_student_bank_details_mvp", {
    p_student_id: studentId
  });
  const row = Array.isArray(data) ? data[0] : data;

  return {
    data: row ? normalizeStudentBankDetails(row) : null,
    error
  };
}

export async function updateStudentFinance(supabase, input) {
  const payload = normalizeStudentFinancePayload(input.form || input, {
    canEditBankDetails: input.canEditBankDetails
  });
  const { data, error } = await supabase.rpc("update_student_finance_mvp", {
    p_student_id: input.studentId,
    p_billing_plan_id: payload.billingPlanId,
    p_monthly_fee_yen: payload.monthlyFeeYen,
    p_postal_address: payload.postalAddress,
    p_replace_bank: payload.replaceBank,
    p_bank_name: payload.bankName ?? null,
    p_bank_code: payload.bankCode ?? null,
    p_branch_name: payload.branchName ?? null,
    p_branch_name_yomigana: payload.branchNameYomigana ?? null,
    p_branch_code: payload.branchCode ?? null,
    p_account_type: payload.accountType ?? null,
    p_account_number: payload.accountNumber ?? null,
    p_account_holder_katakana: payload.accountHolderKatakana ?? null,
    p_billing_start_date: payload.billingStartDate ?? null,
    p_billing_end_date: payload.billingEndDate ?? null
  });
...
export async function fetchRecurringExpenseTemplates(supabase, filters = {}) {
  let query = supabase
    .from("recurring_expense_templates")
    .select(recurringExpenseTemplateSelect)
...
export async function createRecurringExpenseTemplate(supabase, input) {
  const { data, error } = await supabase.rpc("create_recurring_expense_template_mvp", {
    p_school_id: input.schoolId,
    p_category_id: input.categoryId,
    p_name: input.name,
    p_vendor: emptyToNull(input.vendor),
    ...
    p_recurrence: input.recurrence || "monthly",
    p_start_date: emptyToNull(input.startDate),
    p_end_date: emptyToNull(input.endDate),
    p_notes: emptyToNull(input.notes)
  });
...
export async function updateRecurringExpenseTemplate(supabase, input) {
  const { data, error } = await supabase.rpc("update_recurring_expense_template_mvp", {
...
export async function generateRecurringExpenses(supabase, targetMonth) {
  const { data, error } = await supabase.rpc("generate_recurring_expenses_mvp", {
    p_target_month: emptyToNull(targetMonth)
  });

  return { data: data || [], error };
}

export async function fetchFinancialDocuments(supabase, filters = {}) {
  let query = supabase
    .from("financial_documents")
    .select(financialDocumentSelect)
  ...
  const { data, error } = await query;
  const search = filters.search?.trim().toLowerCase();
  const documents = search ? filterFinancialDocumentRows(data || [], search) : data || [];

  return { data: documents, error };
}

export async function fetchFinancialDocument(supabase, financialDocumentId) {
  const { data, error } = await supabase
    .from("financial_documents")
    .select(financialDocumentSelect)
...
export async function fetchFinancialDocumentBadgeCount(supabase) {
  const { count, error } = await supabase
    .from("financial_documents")
    .select("id", { count: "exact", head: true })
    .eq("status", "pending")
...
export async function markFinancialDocumentRead(supabase, financialDocumentId) {
  const { data, error } = await supabase.rpc("mark_financial_document_read_mvp", {
...
export async function dismissFinancialDocument(supabase, financialDocumentId) {
  const { data, error } = await supabase.rpc("dismiss_financial_document_mvp", {
    p_financial_document_id: financialDocumentId
  });
...
export async function createExpenseFromFinancialDocument(supabase, financialDocumentId, input) {
  const { data, error } = await supabase.rpc("create_expense_from_financial_document_mvp", {
    p_financial_document_id: financialDocumentId,
    p_school_id: input.schoolId,
    p_expense_date: emptyToNull(input.expenseDate),
    p_category_id: input.categoryId,
...
export async function createOfficeTodoForFinancialDocument(supabase, financialDocumentId, input = {}) {
  const { data, error } = await supabase.rpc("create_office_todo_for_financial_document_mvp", {
    p_financial_document_id: financialDocumentId,
    p_due_date: emptyToNull(input.dueDate),
    p_assigned_profile_id: emptyToNull(input.assignedProfileId)
  });

  return { data, error };
}

export async function fetchFinancialDocumentRules(supabase, filters = {}) {
  let query = supabase
    .from("financial_document_rules")
    .select(financialDocumentRuleSelect)
    .order("status", { ascending: true })
...
export async function createFinancialDocumentRule(supabase, input) {
  const row = buildFinancialDocumentRuleInsert(input);
  const { data, error } = await supabase
    .from("financial_document_rules")
    .insert(row)
    .select(financialDocumentRuleSelect)
...
export async function fetchOfficeTodos(supabase, filters = {}) {
  let query = supabase
    .from("office_todo_items")
    .select(officeTodoSelect)
    .order("due_date", { ascending: true, nullsFirst: false })
    .order("created_at", { ascending: false })
    .limit(filters.limit || 200);
  ...
  const { data, error } = await query;
  const search = filters.search?.trim().toLowerCase();
  const todos = search ? filterOfficeTodoRows(data || [], search) : data || [];

  return { data: todos, error };
}

export async function fetchOfficeTodoBadgeCount(supabase) {
  const { count, error } = await supabase
    .from("office_todo_items")
    .select("id", { count: "exact", head: true })
    .eq("status", "open");
...
export async function createOfficeTodo(supabase, input) {
  const row = buildOfficeTodoInsert(input);
  const { data, error } = await supabase.rpc("create_office_todo_mvp", {
    p_organization_id: row.organizationId,
    p_school_id: row.schoolId,
    p_title: row.title,
    p_description: emptyToNull(row.description),
    p_due_date: emptyToNull(row.dueDate),
    p_source_type: row.sourceType,
    p_source_reference: emptyToNull(row.sourceReference),
    p_assigned_profile_id: emptyToNull(row.assignedProfileId)
  });

  return { data, error, row };
}

export async function markOfficeTodoCompleted(supabase, todoItemId) {
  const { data, error } = await supabase.rpc("mark_office_todo_completed_mvp", {
    p_todo_item_id: todoItemId
  });
...
export async function fetchPendingTrialBookingImportCount(supabase, filters = {}) {
  ...
  if (filters.reviewStatus === "needs_action") {
    query = query.in("review_status", ["pending_review", "reviewed"]).is("converted_trial_lesson_id", null);
...
export async function dismissPendingTrialBookingImport(supabase, pendingImportId) {
  const patch = { review_status: "dismissed" };
  const { data, error } = await supabase
    .from("pending_trial_booking_imports")
    .update(patch)
    .eq("id", pendingImportId)
...
export function buildPendingTrialBookingReviewPatch(input = {}) {
  return {
    ...
    set_date: normalizeOptionalDate(input.set_date),
    set_time: normalizeOptionalTime(input.set_time),
...
export async function convertTrialLessonProspect(supabase, trialLessonId) {
  const { data, error } = await supabase.rpc("convert_trial_lesson_prospect_to_student", {
    p_trial_lesson_id: trialLessonId
  });
...
export async function updateTrialLessonConversionDetails(supabase, input) {
  const { data, error } = await supabase.rpc("update_trial_lesson_conversion_details_mvp", {
...
export async function deleteTrialLessons(supabase, trialLessonIds) {
  const { data, error } = await supabase.rpc("delete_trial_lessons_mvp", {
    p_trial_lesson_ids: trialLessonIds
  });
...
export async function createStudent(supabase, input) {
  const { data, error } = await supabase.rpc("create_student_with_class_assignment_mvp", {
    p_school_id: input.schoolId,
    ...
    p_contacts: serializeContactRows(input.contacts),
    p_billing_plan_id: emptyToNull(input.billingPlanId),
    p_monthly_fee_yen: normalizeOptionalInteger(input.monthlyFeeYen),
    p_assigned_teacher_profile_id: emptyToNull(input.assignedTeacherProfileId),
    p_lesson_type: input.lessonType,
    p_class_level_id: emptyToNull(input.classLevelId),
    p_create_new_class: Boolean(input.createNewClass),
    p_existing_class_id: emptyToNull(input.classId),
    p_lesson_day: emptyToNull(input.lessonDay),
    p_lesson_time: emptyToNull(input.lessonTime),
...
export async function updateStudent(supabase, input) {
  const { data, error } = await supabase.rpc("update_student_with_class_assignment_mvp", {
    p_student_id: input.studentId,
    p_school_id: input.schoolId,
    p_first_name: input.firstName,
    p_last_name: input.lastName,
...
export async function bulkUpdateStudentStatus(supabase, input, profile) {
  if (!canCreateStudents(profile)) {
    return { data: null, error: new Error("You do not have permission to update student statuses.") };
  }

  const status = String(input?.status || "").trim();
  if (!isValidStudentStatus(status)) {
    return { data: null, error: new Error("Choose a valid student status.") };
  }

  const normalizedStudentIds = normalizeStudentIds(input?.studentIds);
  if (normalizedStudentIds.error) {
    return { data: null, error: normalizedStudentIds.error };
  }

  const studentIds = normalizedStudentIds.data;
  if (!studentIds.length) {
    return { data: null, error: new Error("Select at least one student.") };
  }

  const authResult = await supabase.auth.getUser();
  if (authResult.error || !authResult.data?.user) {
    return { data: null, error: authResult.error || new Error("You must be signed in to update student statuses.") };
  }

  for (const studentId of studentIds) {
    const { data: canManageStudent, error } = await supabase.rpc("can_manage_student", {
      p_student_id: studentId
    });
    ...
    if (canManageStudent !== true) {
      return { data: null, error: new Error("You do not have permission to update one or more selected students.") };
    }
  }
  ...
  if ((data || []).length !== studentIds.length) {
    return { data: null, error: new Error("One or more selected students could not be updated.") };
  }

  return { data, error: null };
}

export async function bulkQueueStudentEmails(supabase, input, profile) {
  if (!canCreateStudents(profile)) {
    return { data: null, error: new Error("You do not have permission to send student email.") };
  }

  const subject = String(input?.subject || "").trim();
  if (!subject) {
    return { data: null, error: new Error("Subject is required.") };
  }
  ...
  const authResult = await supabase.auth.getUser();
  if (authResult.error || !authResult.data?.user) {
    return { data: null, error: authResult.error || new Error("You must be signed in to send student email.") };
  }
  ...
  for (const studentId of studentIds) {
    ...
    if (canManageStudent !== true) {
      return { data: null, error: new Error("You do not have permission to email one or more selected students.") };
    }
  }
  ...
  if ((students || []).length !== studentIds.length) {
    return { data: null, error: new Error("One or more selected students could not be loaded.") };
  }
  ...
  for (const student of students || []) {
    const recipient = getDefaultStudentEmail(student);
...
function normalizeStudentIds(values) {
  ...
  if (normalizedIds.some((value) => !uuidPattern.test(value))) {
    return { data: [], error: new Error("Selected students must use valid student IDs.") };
  }

  return { data: [...new Set(normalizedIds)], error: null };
}
...
function filterFinancialDocumentRows(rows, search) {
  return rows.filter((row) =>
    [
      row.sender,
      row.vendor,
...
function filterOfficeTodoRows(rows, search) {
  return rows.filter((row) =>
    [
      row.title,
      row.description,
