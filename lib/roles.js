...
export const navigationItems = [
  ...
  {
    href: "/expenses/financial-docs/",
    label: "Financial docs to check",
    roles: ["super_admin"]
  },
  {
    href: "/todo/",
...
export function canManageFinancialDocuments(profile) {
  return canManageExpenses(profile);
}

export function canManageOfficeTodos(profile) {
  const roleSet = getRoleSet(profile);
  return ["super_admin", "franchise_owner", "school_manager", "office_staff"].some((role) => roleSet.has(role));
}
...
export function canViewStudentFinance(profile) {
  const roleSet = getRoleSet(profile);
  return ["super_admin", "franchise_owner", "school_manager", "office_staff"].some((role) => roleSet.has(role));
}

export function canViewStudentBankDetails(profile) {
  const roleSet = getRoleSet(profile);
  return ["super_admin", "franchise_owner", "school_manager"].some((role) => roleSet.has(role));
}

export function canEditStudentFinance(profile) {
  return canViewStudentFinance(profile);
}

export function canEditStudentBankDetails(profile) {
  return canViewStudentBankDetails(profile);
}
